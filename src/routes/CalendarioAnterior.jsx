...
const Carousel = () => {
  ...
  return (
    <>
      <div className="ml-4">
        <p className="mt-10 ml-52 font-medium text-xl">
          Selecciona la fecha de tu cita
        </p>
      </div>
      <div className="flex items-center mb-16 mt-2 w-full ">
        <button
          className="p-2 ml-4 bg-gray-300 rounded-full mr-2 hover:-translate-y-1 hover:scale-110 duration-200"
          onClick={goLeft}
        >
          ←
        </button>
        <div
          className={`w-2/5 h-[25rem] bg-azulNav rounded-lg border-2 border-azul transition-transform transform ${
            currentIndex === 0 ? "animate-slideLeft" : "animate-slideRight"
          }`}
        >
            ...
            {diasDelMes.map((dia, index) => (
              <div
                ...
                className={`text-center p-2 ${
                  dia === diaActual
                    ? "bg-azulClaro text-white rounded shadow drop-shadow-md p-2 hover:-translate-y-1 hover:scale-110 duration-200"
                    : "bg-morado text-center text-white rounded shadow drop-shadow-md p-2 hover:-translate-y-1 hover:scale-110 duration-200"
                }`}
              >
                <p>{dia}</p>
              </div>
        ...
        <button
          className="p-2 bg-gray-300 rounded-full ml-2 hover:-translate-y-1 hover:scale-110 duration-200"
          onClick={goRight}
        >
          →
        </button>
        ...
        <div className="w-[32rem] h-[25rem] bg-azulNav ml-4 rounded-lg border-2 border-azul">
          ...
          <div className="flex">
            <div className="mt-4 w-1/2 flex justify-center bg-azul text-white text-bold">
              Corporales
            </div>
            <div className="mt-4 w-1/2 flex justify-center bg-azul text-white text-bold">
              Faciales
        ...
        <div className="w-1/12 h-[25rem] bg-azulNav ml-4 rounded-lg border-2 border-azul flex flex-col justify-between">
          ...
          <div className="flex justify-center mb-2">
            <button className="rounded-2xl bg-turqueza text-white font-bold px-3 py-2 hover:-translate-y-1 hover:scale-100 duration-200">
              RESERVAR
            </button>
          </div>
        </div>
        ...
        {ServCorporal.map((ServCorporal) => (
          <form className="flex justify-left ml-6">
            <label className="p-2 text-justify">
              <input className="mr-4" type="radio" value="opcion" />
